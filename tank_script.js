...
void main() {
    float weight = texture(u_velocityGrid, v_uv).w;
    if (weight < 0.001) {
        outColor = vec4(0.0, 0.0, 0.0, 1.0); // Free surface Dirichlet P=0
        return;
    }
...
void main() {
    ...
    if (!u_isFreeFluid) {
        ...
        // Free Fluid: slight surface tension cohesion based on APIC density proxy
...
function init() {
  ...
  if (divergenceFBO) gl.deleteFramebuffer(divergenceFBO);
  if (pressurePingFBO) gl.deleteFramebuffer(pressurePingFBO);
  if (pressurePongFBO) gl.deleteFramebuffer(pressurePongFBO);
  ...
  divergenceTex = createTexture(gridWidth, gridHeight);
  divergenceFBO = createFBO(divergenceTex);

  pressurePingTex = createTexture(gridWidth, gridHeight);
  pressurePingFBO = createFBO(pressurePingTex);

  pressurePongTex = createTexture(gridWidth, gridHeight);
  pressurePongFBO = createFBO(pressurePongTex);
...
function render() {
  ...
  // 0d: GPU Jacobi Pressure Solver
  gl.viewport(0, 0, gridWidth, gridHeight);
  
  gl.bindFramebuffer(gl.FRAMEBUFFER, pressurePingFBO);
  gl.clearColor(0,0,0,0);
  gl.clear(gl.COLOR_BUFFER_BIT);
  gl.bindFramebuffer(gl.FRAMEBUFFER, pressurePongFBO);
  gl.clearColor(0,0,0,0);
  gl.clear(gl.COLOR_BUFFER_BIT);
  ...
  gl.uniform1i(gl.getUniformLocation(pressureProgram, 'u_velocityGrid'), 2);
  gl.uniform2f(gl.getUniformLocation(pressureProgram, 'u_gridRes'), gridWidth, gridHeight);
  ...
  gl.bindTexture(gl.TEXTURE_2D, prevVelocityGridTex); // contains weight in w channel
  gl.activeTexture(gl.TEXTURE0);
  
  let pPing = pressurePingFBO;
  let pPong = pressurePongFBO;
  let tPing = pressurePingTex;
  let tPong = pressurePongTex;
  
  // Use user-defined iterations, default 60
  let iters = parseInt(document.getElementById('slider-iters')?.value || '60');
...
document.getElementById('slider-iters').addEventListener('change', (e) => {
  PRESSURE_ITERS = parseInt(e.target.value, 10);
});
...
// Tank mode ignores button toggles
// (This is now called inside init())
